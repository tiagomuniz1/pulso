...
- **O lembrete deixa de nomear a clínica**, caindo de cinco placeholders para quatro (`{{1}}` paciente · `{{2}}` profissional · `{{3}}` data · `{{4}}` hora). O nome da clínica já é o *display name* do remetente no celular da paciente, então repeti-lo no corpo era redundante. Com isso `clinicName` sai de `ReminderCandidate` e da projeção SQL — o JOIN com `clinics` **fica**, porque é ele que aplica `is_active` e o soft delete. Se um dia várias clínicas dividirem o mesmo remetente, o campo precisa voltar
